...
public class FindTheBug5 {
    ...
    private static boolean readInput(BufferedReader br) throws IOException {
        ...
        for (int i = 0; i < e; i++) {
            ...
            for(int j = 0; j < portals.size(); j++){
                HauntedHole h = portals.get(j);
                if(h.x1 == y2 && h.y1 == x2){
                    //ours goes into theirs
                    h.x1 = y1;
                    h.y1 = x1;
                }else if(h.x2 == y1 && h.y2 == x1){
                    //thiers goes into ours
                    h.x2 = y2;
                    h.y2 = x2;
                }else{
    ...
    private static boolean relax(long[] distance) {
        ...
        for (HauntedHole portal : portals) {
            ...
            if(distance[vertex1] == INF) continue;
    ...
    private static long[] bellmanFord() {
        ...
        //NO: check for neg cycles only pertaining to our target
        // if (relax(distance)) {
        //     return null;
        // } else {
        //     return distance;
        // }
        long[] newDist = distance.clone();
        for (int iter = 0; iter < (w * h) ; iter++) {
            relax(newDist);
        }
        if(newDist[pointToInt(h-1, w-1)] != distance[pointToInt(h-1, w-1)]){
            return null;
        }else{
            return distance;
        }
